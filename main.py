...
from utils import CosineAnnealingWarmupRestarts2
...
def init_seed(seed):
    ...
    torch.backends.cudnn.benchmark = True   # 为了可以重复训练结果。
...
class Processor():
    ...
    def load_optimizer(self):
        ...
        self.scheduler = CosineAnnealingWarmupRestarts2(self.optimizer,
                                                        first_cycle_steps=self.arg.num_epoch,
                                                        max_lr=self.arg.base_lr,
                                                        min_lr=0.,
                                                        warmup_steps=self.arg.warm_up_epoch,
                                                        gamma=0.1
                                                        )
    ...
    def train(self, epoch, save_model=False):
        ...
        self.print_log('Training epoch: {}, lr: {}'.format(epoch + 1, self.optimizer.param_groups[-1]['lr']))
        ...
        for batch_idx, (data, label, index) in enumerate(process):
            ...
            self.scheduler.step(epoch + batch_idx / len(process))
    ...
    def start(self):
        if self.arg.phase == 'train':
            ...
            self.eval(epoch=0, save_score=True, loader_name=['test'], wrong_file=wf, result_file=rf)  # 总是进行验证
