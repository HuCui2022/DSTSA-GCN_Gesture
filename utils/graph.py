...
class Graph:
    ...
    def __init__(self,
        ...
        assert layout in ['openpose', 'nturgb+d', 'coco', 'custom', 'custom2', 'ucla', 'shrec', 'dhg']
    ...
    def get_layout(self, layout):
        if layout == 'openpose':
            ...
            self.num_node = 22
            inward_ori_index = [(1, 2), (3, 1), (4, 3), (5, 4), (6, 5), (7, 2), (8, 7), (9, 8), (10, 9), (11, 2),
                                (12, 11),
                                (13, 12), (14, 13), (15, 2), (16, 15), (17, 16), (18, 17), (19, 2), (20, 19), (21, 20),
                                (22, 21),
                                (2, 2)]
            self.inward = [(i - 1, j - 1) for (i, j) in inward_ori_index]
            self.center  = 2 - 1
    ...
    def seperated_adjacency(self, with_self=True):
        if self.num_filter <= 3:
            return self.spatial()
        A = self.binary_adj()
        k_list = [i for i in range(self.num_filter)]
        print(k_list)
        assert isinstance(A, np.ndarray)
        ...
        for i in range(len(k_list) - 1):
            k1 = k_list[i]
            k2 = k_list[i + 1]
            Ak.append(np.minimum(np.linalg.matrix_power(A, k2), 1) - np.minimum(np.linalg.matrix_power(A, k1), 1))
...
if __name__ == "__main__":
    ...
    graph = Graph(layout='nturgb+d',
                 mode='random',
                 num_filter=8)
    A_sep1, A_sep2, A_sep3, A_sep4, A_sep5 = graph.A[0], graph.A[1], graph.A[2], graph.A[3], graph.A[4]

    f, ax = plt.subplots(1, 5)
    # ax[0].imshow(A_sep1, cmap='gray')
    # ax[1].imshow(A_sep2, cmap='gray')
    # ax[2].imshow(A_sep3, cmap='gray')
    # ax[3].imshow(A_sep4, cmap='gray')
    # ax[4].imshow(A_sep5, cmap='gray')
    A = np.random.randn(8, 17, 17) * 0.02 + 0.04
    A_sep1, A_sep2, A_sep3, A_sep4, A_sep5 = A[0], A[1], A[2], A[3], A[4]
    ax[0].imshow(A_sep1, cmap='coolwarm')
    ax[1].imshow(A_sep2, cmap='coolwarm')
    ax[2].imshow(A_sep3, cmap='coolwarm')
    ax[3].imshow(A_sep4, cmap='coolwarm')
    ax[4].imshow(A_sep5, cmap='coolwarm')
    plt.show()
