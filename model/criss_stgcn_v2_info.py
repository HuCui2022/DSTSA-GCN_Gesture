...
class criss_gcn_v3(nn.Module):
    ...
    def org_forward(self, x, A=None):
        ...
        # 1 (N), K, 1 (C), 1 (T), V, V
        A = A[None, :, None, None]
        pre_x = self.pre(x).reshape(
            n, self.num_subsets, self.mid_channels, t, v
        )  # self.num_subsets = n
        ...
        if self.ada is not None:
            ...
            if self.comod == "dot":
                ada_graph = torch.einsum("nkctv,nkctw->nkvw", x1, x2) * (
                    (self.mid_channels * t) ** -0.5
                )
                ada_graph = ada_graph[:, :, None, None]
            elif self.comod == "pair-wise":
                ada_graph = torch.einsum("nkctv,nkctw->nktvw", x1, x2)[
                    :, :, None
                ]  # attention 方法
            elif self.comod == "self-pairwise":
                ada_graph = x2.mean(dim=-3, keepdim=True).unsqueeze(-1) - x2.mean(
                    dim=-3, keepdim=True
                ).unsqueeze(-2)

            ada_graph = getattr(self, self.ada_act)(
                ada_graph
            )  # softmax or sigmoid or tanh
            if self.subset_wise:
    ...
    def series_forward(self, x, A=None):
        ...
        if self.ctr is not None:
            ...
            # sat 方法
                ...
                diff = x1.mean(dim=-2, keepdim=True).unsqueeze(-1) - x1.mean(
                    dim=-2, keepdim=True
                ).unsqueeze(
                    -2
                )  # gat 的方法
    ...
    def par_forward(self, x, A=None):
        ...
        if self.ctr is not None:
            ...
            if self.comod == "pair-wise":
                # gat 方法
                diff = x1.mean(dim=-2, keepdim=True).unsqueeze(-1) - x2.mean(
                    dim=-2, keepdim=True
                ).unsqueeze(
                    -2
                )  # gat 的方法
        ...
        if self.ada is not None:
            ...
            if self.comod == "dot":
                ...
                ada_graph = x1.mean(dim=-3, keepdim=True).unsqueeze(-1) - x2.mean(
                    dim=-3, keepdim=True
                ).unsqueeze(
                    -2
                )  # attention 方法
...
class Model(nn.Module):
    def __init__(
        ...
        if self.in_channels != self.base_channels:
            modules = [
                DGBlock(in_channels, base_channels, A.clone(), 1, residual=False)
            ]  # 第一层layer, inc=3,outc=64
        ...
        # add fc_mu, fc_std:
        self.fc_mu = nn.Linear(base_channels, base_channels)
        self.fc_std = nn.Linear(base_channels, base_channels)
        nn.init.xavier_uniform_(self.fc_cls.weight, gain=nn.init.calculate_gain("relu"))
        nn.init.xavier_uniform_(self.fc_mu.weight, gain=nn.init.calculate_gain("relu"))
        nn.init.xavier_uniform_(self.fc_std.weight, gain=nn.init.calculate_gain("relu"))
    ...
    def forward(self, x, y=None):
        ...
        # add VIB
        mu = self.fc_mu(x)
        std = F.softplus(self.fc_std(x) - 5, beta=1)
...
if __name__ == "__main__":
    ...
    flops, params = get_flops(
        net, (3, 180, 20, 1), as_strings=True, print_per_layer_stat=True
    )
    # indim, outdim, stride
