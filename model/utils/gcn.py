...
class dggcn(nn.Module):
    ...
    def forward(self, x, A=None):
        ...
        if self.ctr is not None or self.ada is not None:
            ...
            x1 = self.conv1(tmp_x).reshape(n, self.num_subsets, self.mid_channels, -1, v)       # n k d 1 v
...
class dcggcnv2(nn.Module):
    # dynamic channel-term graph    # add se-c
    def __init__(self,
                 in_channels,
                 out_channels,
                 A,
        ...
        if in_channels != out_channels:
            ...
            self.down = lambda x: x
        self.bn = build_norm_layer(self.norm_cfg, out_channels)[1]
        # self.talk_w = nn.Softmax()
        # self.talk_proj1 = nn.Linear(self.num_subsets, self.num_subsets)
        # self.talk_proj2 = nn.Linear(self.num_subsets, self.num_subsets)
    ...
    def forward(self, x, A=None):
        ...
        if self.ctr is not None or self.ada is not None:
            ...
            tmp_x = x
            if not (self.ctr == 'NA' or self.ada == 'NA'):
                tmp_x = tmp_x.mean(dim=-2, keepdim=True)        # n c t v -> n c 1 v
        ...
        # talking heads
        # A = rearrange(A, 'n k c t v u -> n c t v u k')
        # A = self.talk_proj1(A)
        # # A = self.softmax(A)
        # # A = self.talk_proj2(A)
        # A = rearrange(A, 'n c t v u k -> n k c t v u')
        ...
        # 添加时间维度图, 两种方式进行实现。todo
        # todo 1 : 串联，
        # todo 2: 非pairwise
        # todo 3: 前边的部分也用非pairwise 方式。
        # x4 = self.tanh(pre_x.mean(-3).unsqueeze(-1) - pre_x.mean(-3).unsqueeze(-2))    # n k c t v-> n k t v v
        # x2 = torch.einsum('nktwv, nkctv -> nkctw', x4, pre_x)
...
# fai1, fai2样式。 串联，并联。 2.11 日写完代码。 其他图的构建。 GATV2的构建。
...
class criss_gcn_v3(nn.Module):
    ...
    def forward(self, x, A=None):
        ...
        if self.ctr is not None or self.ada is not None:
            ...
            x1 = self.conv1(tmp_x).reshape(n, self.num_subsets, self.mid_channels, t, v)       # n k d t v
            x2 = self.conv2(tmp_x).reshape(n, self.num_subsets, self.mid_channels, t, v)       # todo: share weights
        ...
        if self.ctr is not None:
            ...
            # diff = x1.unsqueeze(-1) - x2.unsqueeze(-2)          # gat 的方法
            # sat 方法
            diff = torch.einsum('nkdtv, nkdtu-> nkdvu', x1, x2)
        ...
        if self.ada is not None:
            ...
            ada_graph = torch.einsum('nkctv,nkctw->nkvw', x1, x2)[:, :, None, None]      # attention 方法 # todo: 测试稳定性 根号 d
            ada_graph = getattr(self, self.ada_act)(ada_graph)    # softmax or sigmoid or tanh
            if self.subset_wise:
                ada_graph = torch.einsum('nkctuv,k->nkctuv', ada_graph, self.beta)
            else:
